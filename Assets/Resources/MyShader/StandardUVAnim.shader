Shader "MyShader/Standard UV Anim" {
	Properties {
	...
	SubShader{
		...
		Pass {
			...
			float4 frag(VertexOutput IN) : COLOR {
				...
				//float4 mainColorTex = tex2D(m_MainTexture, newUV);
				// 根据_ST的属性，重新计算新的uv。
				// (newUV.xy * m_MainTexture_ST.xy + m_MainTexture_ST.zw)
                //float4 mainColorTex = tex2D(m_MainTexture, TRANSFORM_TEX(newUV, m_MainTexture));
